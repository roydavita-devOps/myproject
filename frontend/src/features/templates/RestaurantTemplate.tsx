...
function RestaurantHero({ website }: { website: Website }) {
  ...
  return (
    <section id="home" className="relative min-h-[82vh] overflow-hidden bg-slate-950">
      <img className="absolute inset-0 size-full object-cover" src={heroImage} alt="" />
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(15,23,42,.86),rgba(15,23,42,.46),rgba(15,23,42,.18))]" />
      <div className="relative mx-auto grid min-h-[82vh] max-w-6xl content-end gap-8 px-4 pb-14 pt-24 md:grid-cols-[1fr_0.42fr] md:items-end md:pb-20">
        <div className="max-w-3xl text-white">
          <p className="tpl-caption mb-3 inline-flex items-center gap-2 font-semibold uppercase text-[var(--tpl-secondary)]">
          ...
          {actions.length > 0 && (
            <div className="mt-8 flex flex-wrap gap-3">
              {actions.slice(0, 2).map((action) => <TemplateButton key={action.href} {...action} />)}
            </div>
          )}
        </div>
        <div className="rounded-lg border border-white/20 bg-white/12 p-5 text-white backdrop-blur">
          <p className="tpl-caption font-semibold uppercase text-[var(--tpl-secondary)]">Today's highlight</p>
          <p className="tpl-h3 tenant-heading mt-3">Fresh menu, clear prices, easy contact.</p>
...
function FeaturedMenu({ items }: { items: NonNullable<Website['menus']> }) {
    ...
    <TemplateSection muted title="Location" description="Bantu pelanggan menemukan lokasi makan atau mengambil pesanan.">
      <div className="grid gap-4 md:grid-cols-[1fr_0.7fr]">
        <TemplateCard>
          ...
          {website.mapsUrl && (
            <div className="mt-5">
              <TemplateButton href={website.mapsUrl} label="Open Maps" icon={<MapPin className="size-4" />} variant="secondary" />
            </div>
          )}
        </TemplateCard>
